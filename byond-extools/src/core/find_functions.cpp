...
#ifdef _WIN32
#warning Win32 build
	FIND_OR_DIE(ExecuteInstruction, "0F B7 48 ?? 8B 78 ?? 8B F1 8B 14 ?? 81 FA ?? ?? 00 00 0F 87 ?? ?? ?? ??")
...
#else
	FIND_OR_DIE(ExecuteInstruction, "0F B7 47 ?? 8B 57 ?? 0F B7 D8 8B 0C ?? 81 F9 ?? ?? 00 00 77 ?? FF 24 8D ?? ?? ?? ??")
